...
shinyServer(function(input, output, session, clientData) {
  ...
  getPDFfilename <- reactive({
    paste0(getShortFileName(), ".pdf")
  })
  
  getExcelfilename <- reactive({
    paste0(getShortFileName(), ".xlsx")
  })
  ...
  observeEvent(input$updateRes, {
    ...
    withProgress({
      ...
      xlsxfilename <- paste0("BVoutput", values$stamp, ".xlsx")
      ...
      cleanPlotFilename <- paste0("cleanResPlot", values$stamp, ".png")
      ...
      dirtyPlotFilename <- paste0("dirtyResPlot", values$stamp, ".png")
